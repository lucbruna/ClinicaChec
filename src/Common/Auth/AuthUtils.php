...
class AuthUtils
{
    ...
    private function notifyIpBlock(string $ip_string): bool
    {
        ...
        if (!empty(OEGlobalsBag::getInstance()->getString('patient_reminder_sender_email')) && !empty(OEGlobalsBag::getInstance()->getString('practice_return_email_path'))) {
            ...
            return MyMailer::emailServiceQueue(OEGlobalsBag::getInstance()->getString('patient_reminder_sender_email'), OEGlobalsBag::getInstance()->getString('practice_return_email_path'), xl('IP Address Block Notification For ClinicaChec Admin'), $message);
        } else {
            error_log("Unable to send ClinicaChec admin email notification since either patient_reminder_sender_email or practice_return_email_path global was not set");
            return false;
        }
    }
    ...
    private function notifyUserBlock(string $username): bool
    {
        ...
        if (!empty(OEGlobalsBag::getInstance()->getString('patient_reminder_sender_email')) && !empty(OEGlobalsBag::getInstance()->getString('practice_return_email_path'))) {
            ...
            return MyMailer::emailServiceQueue(OEGlobalsBag::getInstance()->getString('patient_reminder_sender_email'), OEGlobalsBag::getInstance()->getString('practice_return_email_path'), xl('Username Block Notification For ClinicaChec Admin'), $message);
        } else {
            error_log("Unable to send ClinicaChec admin email notification since either patient_reminder_sender_email or practice_return_email_path global was not set");
            return false;
        }
